...
class T2CInitAllApplications:
    ...
    def __init__(self, arg_clssChrome:Chrome ,arg_dictConfig:dict, arg_clssMaestro:T2CMaestro, arg_botWebbot:WebBot=None, arg_botDesktopbot:DesktopBot=None, arg_clssSqliteQueue:T2CSqliteQueue=None):
        """
    ...
    def add_to_queue(self):
        ...
        chrome = self.var_clssChrome
        chrome.open_browser(arg_strUrl="https://www.dadosmundiais.com/turismo.php")

        # Extrair tabela
        var_listPaisesTable = chrome.extract_table()

        for index, pais in enumerate(var_listPaisesTable):
            var_listItem = []
            var_listItem.append(pais)
            var_listItem.append(index+1)
            self.var_clssSqliteQueue.insert_new_queue_item(arg_strReferencia=pais, arg_listInfAdicional=var_listItem)
    ...
    def execute(self, arg_boolFirstRun=False, arg_clssSqliteQueue=None):
        ...
        for var_intTentativa in range(var_intMaxTentativas):
            try:
                ...
                var_clssChrome = Chrome(arg_botWebbot=self.var_botWebbot, arg_botDesktopbot=self.var_botDesktopbot, arg_clssMaestro=self.var_clssMaestro)

                var_clssChrome.open_browser("https://www.google.com/travel/explore")
